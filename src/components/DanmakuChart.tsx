...
export const DanmakuChart: React.FC<DanmakuChartProps> = ({ data }) => {
    ...
    const handleWheel = (e: WheelEvent<HTMLDivElement>) => {
        ...
        const newStart = start + (deltaY > 0 ? zoomFactor * (end - start) : -zoomFactor * (end - start));
        const newEnd = end - (deltaY > 0 ? zoomFactor * (end - start) : -zoomFactor * (end - start));

        if (newStart < newEnd && newStart >= 0 && newEnd <= data.length - 1) {
            setXDomain([newStart, newEnd]);
        }
    };

    return (
        <div style={{ width: '100%', height: 400 }} onWheel={handleWheel}>
             <ResponsiveContainer>
                <AreaChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                    ...
                    <XAxis 
                        ...
                        tickFormatter={(timeStr, index) => {
                            ...
                            return data[Math.floor(index)]?.displayTime.substring(0, 5) || '';
                        }}
                    />
